...
export const Card: React.FC<CardProps> = ({ card, isRevealed, onClick, className = "w-24 h-40 sm:w-32 sm:h-52", style, language }) => {
  return (
    <div 
      ...
      <div className={`relative w-full h-full duration-700 transform-style-3d transition-all ${isRevealed ? 'rotate-y-180' : ''}`}>
        ...
        <div className="absolute w-full h-full backface-hidden rounded-lg shadow-xl border-2 border-purple-900 bg-slate-800 flex items-center justify-center overflow-hidden group">
          ...
          <div className="absolute inset-2 border border-purple-500/30 rounded flex items-center justify-center">
             <div className="w-12 h-12 rounded-full border border-amber-500/40 flex items-center justify-center relative">
          ...
          {/* Mystery Symbol */}
          <span className="font-mystic text-4xl text-purple-200/60 transition-transform duration-500 group-hover:scale-110">☾</span>
        </div>
        ...
        <div className={`absolute w-full h-full backface-hidden rotate-y-180 rounded-lg shadow-[0_0_20px_rgba(234,179,8,0.2)] bg-slate-900 border-2 border-amber-600 overflow-hidden flex flex-col`}>
           {card && (
             <>
                ...
                <div className="h-6 sm:h-8 bg-slate-950 border-t border-amber-800 flex items-center justify-center px-1">
                  <span className={`text-[9px] sm:text-[10px] uppercase tracking-wider font-bold truncate ${card.isReversed ? 'text-purple-400' : 'text-amber-500'}`}>
